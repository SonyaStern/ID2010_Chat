        ...
        Runnable            // for the distribution thread.
{
     ...
     * modified.
     */
    protected Map<RemoteEventListener, LocalDateTime> clients =
            new HashMap<>();
     ...
     */
    protected void addClient(RemoteEventListener rel) throws UnknownEventException, RemoteException {
        for (RemoteEventListener el : clients.keySet()) {
            el.notify(new ChatNotification(this, "Client entered the chat " + rel.toString(), msgCount));
        }
        synchronized (clients) {
            clients.put(rel, LocalDateTime.now());
        }
    ...
    @Override
    public void register(RemoteEventListener rel) throws RemoteException, UnknownEventException {
        if (rel != null) {
            addClient(rel);
        }
    }
    ...
    @Override
    public void unregister(RemoteEventListener rel) throws RemoteException, UnknownEventException {
        if (rel != null) {
            removeClient(rel);
        }
    }
    ...
    public void readLoop() {
        ...
        while (!halted) {
            ...
            if (arg.equalsIgnoreCase("quit") ||
                ...
                halted = true;
            } else if (arg.equalsIgnoreCase("clients")) {
